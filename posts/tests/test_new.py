...
работает неправильно. Ошибка: `{e}`'''
        ...
        assert response.status_code in (301, 302), (
            'Проверьте, что со страницы `/new/`'
            ' после создания поста перенаправляете на главную страницу')
        ...
        response = user_client.post(url, data={'text': text})
        assert response.status_code in (301, 302), (
            'Проверьте, что со страницы `/new/` после создания поста'
            ' перенаправляете на главную страницу')
        post = Post.objects.filter(author=user, text=text,
        ...
        assert response.status_code == 200, (
            'Проверьте, что на странице `/new/` выводите ошибки при'
            ' неправильной заполненной формы `form`')
