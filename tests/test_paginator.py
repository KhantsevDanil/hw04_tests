...
работает неправильно. Ошибка: `{e}`'''
    ...
    def test_index_paginator_view_get(self, client, post_with_group):
        ...
        assert response.status_code != 404, 'Страница `/`' \
                                            ...
                                            ' этот адрес в *urls.py*'
        assert 'paginator' in response.context, (
            'Проверьте, что передали переменную `paginator` в'
            ' контекст страницы `/`'
        )
        assert type(response.context['paginator']) == Paginator, (
            'Проверьте, что переменная `paginator` на странице'
        ...
        assert 'page' in response.context, (
            'Проверьте, что передали переменную `page` в'
            ' контекст страницы `/`'
        )
        assert type(response.context['page']) == Page, (
            'Проверьте, что переменная `page` на странице'
