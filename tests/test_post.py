    ...
    def test_post_view_get(self, client, post_with_group):
        try:
            response = client.get(f'/{post_with_group.author.username}/'
                                  f'{post_with_group.id}')
        except Exception as e:
            assert False, f'''Страница `/<username>/<post_id>/`
работает неправильно. Ошибка: `{e}`'''
        if response.status_code in (301, 302):
            response = client.get(f'/{post_with_group.author.username}/'
                                  f'{post_with_group.id}/')
        assert response.status_code != 404, (
            'Страница `/<username>/<post_id>/` не'
            ' найдена, проверьте этот адрес в *urls.py*'
        )

        profile_context = get_field_context(
            response.context, get_user_model())
        assert profile_context is not None, (
            'Проверьте, что передали автора в'
            ' контекст страницы `/<username>/<post_id>/`'
        )
...
class TestPostEditView:
    ...
    def test_post_edit_view_get(self, client, post_with_group):
        try:
            response = client.get(f'/{post_with_group.author.username}/'
                                  f'{post_with_group.id}/edit')
        except Exception as e:
            assert False, f'''Страница `/<username>/<post_id>/edit/`
работает неправильно. Ошибка: `{e}`'''
        if (
                response.status_code in (301, 302)
                and not
                response.url.startswith(f'/'
                                        f'{post_with_group.author.username}/'
                                        f'{post_with_group.id}')
        ):
            response = client.get(f'/{post_with_group.author.username}/'
                                  f'{post_with_group.id}/edit/')
        assert response.status_code != 404, (
            'Страница `/<username>/<post_id>/edit/` не найдена,'
            ' проверьте этот адрес в *urls.py*'
        )

        assert response.status_code in (301, 302), (
            'Проверьте, что вы переадресуете'
            ' пользователя со страницы '
            '`/<username>/<post_id>/edit/` на страницу поста,'
            ' если он не автор'
        )
    ...
    def test_post_edit_view_author_get(self, user_client, post_with_group):
        try:
            ...
            assert False, f'''Страница `/<username>/<post_id>/edit/` работает
неправильно. Ошибка: `{e}`'''
        if response.status_code in (301, 302):
            response = user_client.get(f'/{post_with_group.author.username}/'
                                       f'{post_with_group.id}/edit/')
        assert response.status_code != 404, (
            'Страница `/<username>/<post_id>/edit/` не найдена, проверьте'
        ...
        assert post_context is not None, (
            'Проверьте, что передали статью в контекст страницы'
            ' `/<username>/<post_id>/edit/` типа `Post`'
        )

        assert 'form' in response.context, (
            'Проверьте, что передали форму `form` в контекст страницы'
            ' `/<username>/<post_id>/edit/`'
        )
        assert len(response.context['form'].fields) == 2, (
            'Проверьте, что в форме `form` на страницу'
            ' `/<username>/<post_id>/edit/` 2 поля'
        )
        ...
        assert type(response.context['form'].fields['text']) == (
...
работает неправильно. Ошибка: `{e}`'''
        url = (
            ...
            if response.status_code in (301, 302)
            else f'/{post_with_group.author.username}/'
                 f'{post_with_group.id}/edit'
        )
        ...
        assert post is not None, (
            'Проверьте, что вы изминили пост при'
            ' отправки формы на странице `/<username>/<post_id>/edit/`'
        )
        assert response.url.startswith(f'/'
                                       f'{post_with_group.author.username}/'
                                       f'{post_with_group.id}'), (
